    ...
    <div class="lt-register">
        <Card class="card" :bordered="false">
            ...
            <Form ref="form" :model="model" :rules="rules">
                ...
                <FormItem prop="email">
                    <Input size="large" type="email" v-model="model.email" placeholder="邮箱地址">
                        <Icon type="ios-mail" slot="prepend"/>
                    </Input>
                </FormItem>
                <FormItem prop="password">
                    <Input size="large" type="password" v-model="model.password" placeholder="登录密码">
                ...
                <FormItem>
                    <Button size="large" type="success" @click="handleSubmit()">注册</Button>
...
export default {
    ...
    methods: {
        ...
        async onSendMsgCode() {
            ...
            this.verifyText = "发送中...";
            const resp = await api.request('/api/ltshop/bapi/send_verify_code', this.model);
            if (resp.status === 0) {
        ...
        async handleSubmit() {
            ...
            const resp = await api.request('/api/ltshop/bapi/register', {model:this.model, verifyCode:this.model.verifyCode});
            if (resp.status === 0) {
                this.$Modal.success( {title:'注册成功', content:'蓝途速搭欢迎您!', 
                        onOk: ()=>{
                            this.$router.push('/user');
                        }
                    });
            } else {
                this.$Modal.error( {title:'注册失败', content:resp.message});
                this.reloadImageCode();
            }
        }
    }
